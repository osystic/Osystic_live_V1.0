...
export default function PaymentsPage() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["payments"], queryFn: async () => { const r = await fetch("/api/admin/finance/payments"); return r.json(); } });

  const create = useMutation({ mutationFn: async (f: typeof form) => { const r = await fetch("/api/admin/finance/payments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(f) }); if (!r.ok) throw new Error("Failed"); return r.json(); },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["payments"] }); setShowForm(false); toast.success("Payment recorded"); setForm({ entityId: "", amount: 0, paymentDate: "", paymentMethod: "Bank Transfer", reference: "", notes: "" }); },
  ...
  return (
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-gray-900">Payments Received</h1><p className="text-gray-500 mt-1">Track customer payments against invoices</p></div>
        <button onClick={() => setShowForm(!showForm)} className="flex items-center gap-2 bg-green-600 text-white px-4 py-2 rounded-lg hover:bg-green-700"><Plus size={18} /> Record Payment</button>
      </div>
      {showForm && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <h3 className="text-lg font-semibold mb-4">Record Customer Payment</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input placeholder="Invoice ID" value={form.entityId} onChange={e => setForm({ ...form, entityId: e.target.value })} className="border rounded-lg px-3 py-2" />
            ...
            <select value={form.paymentMethod} onChange={e => setForm({ ...form, paymentMethod: e.target.value })} className="border rounded-lg px-3 py-2"><option>Bank Transfer</option><option>PayPal</option><option>Stripe</option><option>Cash</option></select>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200">
        {isLoading ? <div className="p-6 text-center text-gray-500">Loading...</div> : payments.length === 0 ? (
          <div className="p-6 text-center text-gray-500"><CreditCard size={48} className="mx-auto text-gray-300 mb-2" />No payments yet.</div>
        ) : (
            ...
            <tbody className="divide-y divide-gray-100">
              {payments.map((p: { id: string; paymentNumber: string; amount: number; paymentMethod: string; paymentDate: string; status: string }) => (
                <tr key={p.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-sm text-green-600 font-semibold">${Number(p.amount).toLocaleString()}</td>
